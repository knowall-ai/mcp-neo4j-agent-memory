...
. Run *gpt-image-2* (the `gen-image` default: edits endpoint, `input_fidelity` high) with *two reference images*: `docs/brand/reverie-wordmark.png` for the lettering style and `docs/brand/reverie-wordmark-logo-a.png` (the original Logo A crop) as the second. Ask for "the exact same outlined neon-green display lettering, letter for letter, on a dark plate, no other text, no icons, spelling exact" and add the line "letters about 1.5x wider than tall, squared, thin double-stroke, dark fill". Lesson from the PRESENCE wordmark (2026-09-05): gpt-image-1 kept producing condensed letters (per-letter width-to-height about 0.5 against REVERIE's 1.2 to 1.5) whatever the prompt said, and gpt-image-1.5 came out heavier and brighter than the family; gpt-image-2 reproduced the squared extended double-stroke alphabet first time.
. Verify the proportions with a per-letter measurement: split the lettering into glyphs on the bright core strokes (luminance above 170, gaps of 14 px or more), not on alpha, because the glow bridges neighbouring letters. Each glyph should be roughly as wide as, or wider than, it is tall.
...
| Wordmark
| Centred; width *53 %* of banner width (its natural size at 1536 px); top edge at 6 % of the banner height (62 px at 1024).
...
| `brand/reverie-wordmark-logo-a.png`
| The original Logo A lettering crop (950 x 220, on its dark plate); the second reference image when generating a sister wordmark.
