...
. Use the compositing script below with that cut-out; leave the stroke weight as generated. The glow is baked into the render: the recipe adds no blur to the wordmark.
...
| Subtitle
| Exactly the *same width as the wordmark lettering*; height *0.36 x* the lettering height (squat and bold, stretched with Lanczos); placed *0.60 x* subtitle height below the lettering.
